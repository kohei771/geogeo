...
def compute_intensity_meanvar(intensity, knn_indices):
    # intensity: (N_f,), knn_indices: (N_c, K)
    meanvar = []
    for i in range(knn_indices.shape[0]):
        knn_idx = knn_indices[i]
        knn_idx = torch.clamp(knn_idx, 0, intensity.shape[0] - 1)
        knn_intensity = intensity[knn_idx]
        ...
        if valid_mask.any().item():
            ...
            mean_val = torch.tensor(0.0, device=intensity.device)
            var_val = torch.tensor(0.0, device=intensity.device)
        meanvar.append(torch.stack([mean_val, var_val]))
    return torch.stack(meanvar)  # (N_c, 2)
...
class GeoTransformer(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        ref_meanvar = compute_intensity_meanvar(ref_intensity, ref_node_knn_indices)  # (N_c, 2)
        # src側
        src_intensity = feats_f[ref_length_f:][:, 0]
        src_meanvar = compute_intensity_meanvar(src_intensity, src_node_knn_indices)  # (N_c, 2)
