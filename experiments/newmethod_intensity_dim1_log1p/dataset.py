...
from geotransformer.datasets.registration.newmethod_intensity_dim1_log1p.dataset import NewMethodIntensityPairDataset
...
class IntensityOnlyDataset(NewMethodIntensityPairDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.use_intensity and ref_points.shape[1] >= 4:
            data_dict['ref_feats'] = np.log1p(ref_points[:, 3:4].astype(np.float32))  # Only intensity, log1p変換
        else:
            # If no intensity, use dummy features
            data_dict['ref_feats'] = np.ones((ref_points.shape[0], 1), dtype=np.float32)
            
        if self.use_intensity and src_points.shape[1] >= 4:
            data_dict['src_feats'] = np.log1p(src_points[:, 3:4].astype(np.float32))  # Only intensity, log1p変換
        else:
            # If no intensity, use dummy features
