...
#ここ追加した部分
def save_geometries_as_image(filename, *geometries, width=1024, height=768, bg_color=(1,1,1,1)):
    import open3d as o3d
    # Open3D >=0.12
    try:
        ...
        for g in geometries:
            name = str(id(g))
            renderer.scene.add_geometry(name, g, o3d.visualization.rendering.MaterialRecord())
...
def save_pointclouds_as_2d_image(filename, *pointclouds, colors=None):
    fig = plt.figure()
    ax = fig.add_subplot(111)
    for i, pcd in enumerate(pointclouds):
        pts = np.asarray(pcd.points)
        c = colors[i] if colors is not None else None
        ax.scatter(pts[:, 0], pts[:, 1], s=1, alpha=0.7, label=f'pcd{i}', c=c)
    ax.set_aspect('equal')
    plt.legend()
